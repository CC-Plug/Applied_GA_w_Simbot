...
def after_simulation(simbot: Simbot):
    ...
    for robot in simbot.robots:
        ...
        Logger.info(f"Final Smell Sensor: {robot.target}")
    ...
    # Adding Fitness value after sorting
    weighted_fitnesss_list = [robot.fitness for robot in simbot.robots]
    
    Logger.info(f"Weighted_Fitness_list: \n {weighted_fitnesss_list}")
    ...
    def select():
        ...
        # Pick up with weight random
        # range count from 0 to number robots, k = number of pick
        Logger.info(f"Range = {[x for x in range(num_robots)]}")
        Logger.info(f"Weight List: {weighted_fitnesss_list}")
        new_index = random.choices([x for x in range(num_robots)], weights=weighted_fitnesss_list, k=2)
        Logger.info(f"Type of data output : {type(new_index)}")
        Logger.info(f"Result of Picked up robot: {new_index}")
